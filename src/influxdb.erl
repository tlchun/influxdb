...
-export([write/2, is_running/1]).

-record(state, {
  write_protocol = udp :: http | udp,
...
start_link(Opts) ->
  gen_server:start_link(influxdb, [Opts], []).

-spec write(Pid, Points) -> ok |
{erro, atom()} when Pid :: pid(), Points :: [Point],
  Point :: #{measurement := atom() |binary() |list(),
  tags => map(),
  fields := map(),
  timestamp => integer()}.

write(Pid, Points) -> gen_server:cast(Pid, {write, Points}).

-spec is_running(pid()) -> boolean().
is_running(Pid) -> gen_server:call(Pid, is_running).

init([Opts]) ->
  WriteProtocol = get_value(write_protocol, Opts, udp),
  BatchSize = get_value(batch_size, Opts, 32),
  UDPOpts = merge_default_opts(get_value(udp_opts, Opts, []), [{host, "127.0.0,1"}, {port, 8089}]),
  HTTPOpts0 = merge_default_opts(get_value(http_opts, Opts, []),
    [{host, "127.0.0.1"},
      {port, 8086},
      {database, "mydb"},
      {precision, ms},
  ...
  HTTPOpts = lists:keyreplace(precision,
    1,
    [{url, URL} | HTTPOpts0],
    {precision, to_string(get_value(precision, HTTPOpts0))}),
  State = #state{write_protocol = WriteProtocol, batch_size = BatchSize, http_opts = HTTPOpts, udp_socket = undefined},
  case WriteProtocol of
    udp ->
      ...
      {ok, State#state{udp_opts = lists:keyreplace(host, 1, UDPOpts, {host, IPAddress}), udp_socket = Socket}};
    http -> {ok, State}
  end.
    ...
    Data ->
      case WriteProtocol of
        ...
        http ->
          ...
          QueryParams = may_append_authentication_params([{"db", get_value(database, HTTPOpts)}, {"precision", get_value(precision, HTTPOpts)}], HTTPOpts),
          HTTPOptions = case get_value(https_enabled, HTTPOpts) of
                          false ->
                            [{ssl, get_value(ssl, HTTPOpts)}];
                          true -> []
                        end,
          NewURL = append_query_params_to_url(URL, QueryParams),
          ...
          of
            {ok, {{_, 204, _}, _, _}} ->
              logger:debug("[InfluxDB] Write ~p successfully",
                [NPoints]);
            {ok, {{_, StatusCode, ReasonPhrase}, _, Body}} ->
              logger:error("[InfluxDB] Write ~p failed: ~p ~s, Details: ~s",
                [NPoints,
                  StatusCode,
                  ReasonPhrase,
                  Body]);
            {error, Reason} ->
              logger:error("[InfluxDB] Write ~p failed: ~p",
                [NPoints, Reason])
          end
      end
  end,
...
make_url(HTTPOpts) ->
  Host = binary_to_list(case
                          list_to_binary(get_value(host, HTTPOpts))
                        of
...
merge_default_opts(Opts, []) -> maps:to_list(Opts);
...
getaddr(Host) ->
  case getaddr(Host, inet) of
    {ok, AddressFamily, IPAddress} ->
      {ok, AddressFamily, IPAddress};
    {error, _Reason} -> getaddr(Host, inet6)
  end.

getaddr(Host, AddressFamily)
  when AddressFamily =:= inet orelse
  AddressFamily =:= inet6 ->
  case inet:getaddr(Host, AddressFamily) of
    {ok, IPAddress} -> {ok, AddressFamily, IPAddress};
    {error, Reason} -> {error, Reason}
  end.

may_append_authentication_params(QueryParams0, AuthParams) ->
  QueryParams = [{"u", get_value(username, AuthParams, undefined)}, {"p", get_value(password, AuthParams, undefined)} | QueryParams0],
...
drain_points(Cnt, Acc) ->
  ...
  after 0 -> lists:append(lists:reverse(Acc))
  end.
